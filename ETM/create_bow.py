...
def create_bow2word():
    docs = []
    for i in range(len(bow_tokens)):
        words = []
        for j in range(len(bow_tokens[i])):
            words.append(id2word[bow_tokens[i][j]])
        docs.append(words)
    data = {"bow2word": docs, "word2id": word2id, "id2word": id2word}
    with open(path_save + 'bow2word.pkl', 'wb') as f:
        pickle.dump(data, f)
    return docs
...
bow_tokens, bow_counts = split_bow(bow_all, n_docs_all)
savemat(path_save + 'bow_all_tokens.mat', {'bow_tokens': bow_tokens}, do_compression=True)
savemat(path_save + 'bow_all_counts.mat', {'bow_counts': bow_counts}, do_compression=True)
create_bow2word()
print('Data ready !!')
print('*************')
